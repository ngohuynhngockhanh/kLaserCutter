...
var SVGReader = {
  ...
  parse : function(svgstring, config) {
    config = config || {};
	config.unitConst = config.unitConst || 1;
    this.SVGAttributeMapping.unitConst = config.unitConst; // fix convert to svg online
	this.config = config;
	console.log(config.unitConst);
	//this.defaultXform = [config.unitConst, 0, 0, config.unitConst, 0, 0];
          ...
          if (tag.tagName == 'text') {//convert text to path
			...
			for (var j = 0; j < tspanList.length; j++) {
				...
				if (phpjs.strlen(phpjs.trim(text)) == 0)
					continue;
				var textX = tspan._attrsByQName.x.data;
				var textY = tspan._attrsByQName.y.data;
    ...
    //	console.log(val);
      ...
      for (var i=0; i<segs.length; i++) {
        ...
        if (xformKind == 'translate') {
          if (params.length == 1) {
          	//params[0] *= this.unitConst;
            xforms.push([1, 0, 0, 1, params[0], params[0]])
          } else if (params.length == 2) {
          	//params[0] *= this.unitConst;
          	//params[1] *= this.unitConst;
