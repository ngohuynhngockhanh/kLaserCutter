      ...
      if (tag.childNodes) {
        if (tag.tagName) {
          ...
          if (tag.tagName == 'text') {//convert text to path
          	var tspan = tag.querySelector('tspan'); 
          	var text = phpjs.trim(tspan.innerHTML);
          	if (phpjs.strlen(text) == 0)
          		continue;
          	var textX = tag._attrsByQName.x.data;
          	var textY = tag._attrsByQName.y.data;
          	...
          	var fontSize = phpjs.floatval(tspanstyle['font-size']);
